...
def make_token(fingerprint: str = "fp:secret-1", user_id: uuid.UUID = USER_ID) -> RefreshToken:
...
async def test_added_token_is_retrievable_after_commit(uow: FakeSessionsUnitOfWork) -> None:
    token = make_token()
    async with uow:
...
async def test_changes_are_rolled_back_without_commit(uow: FakeSessionsUnitOfWork) -> None:
    token = make_token()
    async with uow:
        await uow.refresh_tokens.add(token)
...
async def test_revoke_succeeds_once_and_then_reports_no_op(
    ...
    async with uow:
        assert await uow.refresh_tokens.revoke(token.id, revoked_at=NOW) is True
        await uow.commit()

    async with uow:
        assert await uow.refresh_tokens.revoke(token.id, revoked_at=NOW) is False


async def test_revoking_an_unknown_token_reports_no_op(uow: FakeSessionsUnitOfWork) -> None:
    async with uow:
        assert await uow.refresh_tokens.revoke(uuid.uuid4(), revoked_at=NOW) is False


async def test_revoked_token_is_no_longer_usable(uow: FakeSessionsUnitOfWork) -> None:
    token = make_token()
    async with uow:
        await uow.refresh_tokens.add(token)
        await uow.commit()
    async with uow:
        await uow.refresh_tokens.revoke(token.id, revoked_at=NOW)
        await uow.commit()
...
async def test_revoke_all_returns_the_number_actually_revoked(
    ...
    tokens = [make_token(f"fp:secret-{i}") for i in range(3)]
    async with uow:
        for token in tokens:
            await uow.refresh_tokens.add(token)
        await uow.commit()
    ...
    async with uow:
        await uow.refresh_tokens.revoke(tokens[0].id, revoked_at=NOW)
        await uow.commit()

    async with uow:
        # 已撤銷的那一筆不該再被計入
        assert await uow.refresh_tokens.revoke_all_for_user(USER_ID, revoked_at=NOW) == 2
        await uow.commit()


async def test_revoke_all_can_keep_the_current_session(uow: FakeSessionsUnitOfWork) -> None:
    """05_spec 第 2.1 節:改密碼撤銷其他 session,但不把使用者自己踢出去。"""
    keep, other = make_token("fp:keep"), make_token("fp:other")
    async with uow:
    ...
    async with uow:
        revoked = await uow.refresh_tokens.revoke_all_for_user(
            USER_ID, revoked_at=NOW, except_token_id=keep.id
        )
        await uow.commit()
    assert revoked == 1
...
async def test_revoke_all_only_touches_the_given_user(uow: FakeSessionsUnitOfWork) -> None:
    mine, theirs = make_token("fp:mine"), make_token("fp:theirs", uuid.uuid4())
    async with uow:
    ...
    async with uow:
        assert await uow.refresh_tokens.revoke_all_for_user(USER_ID, revoked_at=NOW) == 1
        await uow.commit()
