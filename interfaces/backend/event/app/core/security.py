...
Event 只**驗證** Auth 服務簽發的 access token,不簽發也不換發——簽章金鑰與 token
生命週期規則(02_spec_登入註冊與密碼重設.md 第 2.3 節)屬於 Auth 服務的職責。
"""
...
async def require_internal_caller(
    ...
    """跨服務內部端點(規格審查 #8):DELETE /internal/devices/{device_id}/events。
