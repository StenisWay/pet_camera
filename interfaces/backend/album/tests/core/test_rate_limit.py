...
from tests.domains.album.fakes import InMemoryRateLimitCounter
...
async def test_requests_under_the_limit_pass(counter):
    ...
    for _ in range(3):
        await limiter.check("user-1", endpoint="GET /media")
...
async def test_counting_is_per_identity_and_endpoint(counter):
    ...
    await limiter.check("user-2", endpoint="GET /media")
    await limiter.check("user-1", endpoint="DELETE /media/{id}")
